...
def _payment_entry_filters(d):
    account = d.get("account")
    voucher_no = d.get("voucher_no")
    result = frappe.db.get_value(
        "Payment Entry", {"name": voucher_no, "paid_to": account}, "remarks"
    )
    if result is None:
        result = frappe.db.get_value(
            "Payment Entry", {"name": voucher_no, "paid_from": account}, "remarks"
        )
    return result

def _purchase_invoice_filters(d):
    voucher_no = d.get("voucher_no")
    account = d.get("account")

    # Check if this account matches the header-level payable account (credit_to)
    pi = frappe.db.get_value(
        "Purchase Invoice", voucher_no, ["credit_to", "remarks"], as_dict=True
    )
    if pi and account == pi.credit_to:
        return pi.remarks

    # Otherwise fall back to item-level expense_account match
    return frappe.db.get_value(
        "Purchase Invoice Item",
        {"parent": voucher_no, "expense_account": account},
        "description",
    )

VOUCHER_DESCRIPTION_CONFIG = {
    "Sales Invoice": {
        "doctype": "Sales Invoice Item",
        "filters": lambda d: {"parent": d.get("voucher_no"), "income_account": d.get("account")},
        "fieldname": "description",
    },
    "Delivery Note": {
        "doctype": "Delivery Note Item",
        "filters": lambda d: {"parent": d.get("voucher_no"), "expense_account": d.get("account")},
        "fieldname": "description",
    },
    "Journal Entry": {
        "doctype": "Journal Entry",
        "filters": lambda d: {"name": d.get("voucher_no")},
...
def get_result_as_list(data, filters):
	...
	for d in data:
		...
		if not d.get('bill_no'):
			d['bill_no'] = get_bill_no(d)
		set_voucher_description(d)

	return data

def set_voucher_description(d):
    voucher_type = d.get("voucher_type")

    if voucher_type == "Purchase Invoice":
        d["description"] = _purchase_invoice_filters(d)
        return

    if voucher_type == "Payment Entry":
        d["description"] = _payment_entry_filters(d)
        return

    config = VOUCHER_DESCRIPTION_CONFIG.get(voucher_type)
    if config:
        d["description"] = frappe.db.get_value(
            config["doctype"], config["filters"](d), config["fieldname"]
        )
...
def get_bill_no(d):
    ...
    if not meta.has_field("bill_no"):
        return None

    return frappe.get_value(voucher_type, voucher_no, "bill_no")
